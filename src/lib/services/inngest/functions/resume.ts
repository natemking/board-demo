...
import { updateUserResume } from 'lib/db/userResumes';
...
export const createAiSummaryOfUploadedResume = inngest.createFunction(
    ...
    async ({ step, event }) => {
        ...
        if (!userResume) return undefined;

        const result = step.ai.infer('create-ai-summary', {
                ...
                messages: [
                    {
                        ...
                        content: [
                            {
                                type: 'document',
                                source: { type: 'url', url: userResume.resumeFileUrl },
                            },
                            {
                                type: 'text',
        ...
        await step.run('save-ai-summary', async () => {
            const message = (await result).content[0]
